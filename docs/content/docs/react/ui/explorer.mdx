...
<Callout type="info">The URL is the single source of truth. All navigation actions update the URL first, then components react to URL changes.</Callout>
...
<Callout type="warn">Virtual files must never be passed to file operations like copy, move, or delete. Always check `isVirtualFile()` before backend mutations.</Callout>
...
<Callout type="info">Apply this pattern consistently to `file_system`, `disk_type`, `volume_type`, `form_factor`, and any other backend enum fields.</Callout>
