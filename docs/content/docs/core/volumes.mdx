...
<Callout type="info">
See [Library Sync](/core/library-sync) for details on how ownership flows through volumes and enables seamless device transfers.
</Callout>
...
These URIs work identically to local volume paths and enable seamless operations across all storage backends. See [Unified Addressing](/core/addressing) for complete details.
