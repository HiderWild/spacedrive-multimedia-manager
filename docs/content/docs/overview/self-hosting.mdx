...
description: Deploy the Spacedrive server for remote access and headless operation.
---
...
<Callout type="warn">
	Production deployments must set `SD_AUTH`. The server refuses to start
	without authentication unless explicitly disabled. This prevents accidental
	exposure of your libraries.
</Callout>
...
<Callout type="info">
	The server never modifies your original files. Indexing is read-only. File
	operations like copy and move require explicit user action through connected
	clients.
</Callout>
