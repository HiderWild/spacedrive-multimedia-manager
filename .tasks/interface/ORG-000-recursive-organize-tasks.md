...
| ORG-INT-01 | Vertical smoke flow for create, mark, review, commit, restart, and drift | ORG-FE-04, ORG-BE-05 | Integration tests and minimal test fixtures | Verified | coordinator | Final shared-wrapper run: commit lib 8/8, decision repository 4/4, repository 30/30, wire flow 3/3, preview 4/4; Tauri legacy 4/4; frontend organize/legacy 30/30. No live browser/release-bundle smoke is available in this environment |
...
| ORG-VERIFY-02 | Final build, focused suites, type drift, task validation, and Windows smoke evidence | ORG-INT-01 | Read-only verification | Blocked | coordinator | Shared-wrapper Rust suites and Tauri legacy tests pass, frontend organize/legacy is 30/30, and `task-validator validate` passes. Full interface typecheck remains blocked by pre-existing missing assets/package declarations; live browser/release-bundle smoke is unavailable |
...
- [ ] Focused unit, contract, integration, generated-type, task-validator, and Windows smoke checks pass. Final shared-wrapper Rust suites, Tauri legacy tests, task validation, and frontend organize tests pass. Full interface typecheck is blocked by pre-existing missing `packages/assets` image/SVG modules plus unrelated `maplibre-gl` and `@spacebot/api-client` declarations; live browser/release-bundle smoke is unavailable.
