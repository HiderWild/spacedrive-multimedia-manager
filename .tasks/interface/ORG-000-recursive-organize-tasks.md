...
| ORG-INT-01 | Vertical smoke flow for create, mark, review, commit, restart, and drift | ORG-FE-04, ORG-BE-05 | Integration tests and minimal test fixtures | Candidate | coordinator | WebDriver harness migrated in `80ae1ac53`, PathBar entry in `29509f748`, task-record cleanup and image/video-shaped fixtures in `3c2c08963`, and stable UI selectors plus Ctrl-lasso/parent-progress/Finish-confirmation coverage in `048fd5d04`; it covers lasso/conflict/reload/drift-gated commit/disk effects/lifecycle, with legacy dead code removed in `a49502b63` and Windows snapshot fixtures added in `201988ca3`; static Python checks pass, but live execution is unavailable because Selenium is not installed and no WebView2 DevTools app target is running |
