...
| ORG-INT-01 | Vertical smoke flow for create, mark, review, commit, restart, and drift | ORG-FE-04, ORG-BE-05 | Integration tests and minimal test fixtures | Candidate | coordinator | WebDriver harness migrated in `80ae1ac53`, expanded for lasso/conflict/reload/drift-gated commit/disk effects/lifecycle, and legacy dead code removed in `a49502b63`; Windows snapshot fixtures added in `201988ca3`; static Python checks pass, but live execution is unavailable because Selenium is not installed and no WebView2 DevTools app target is running |
...
| ORG-VERIFY-02 | Final build, focused suites, type drift, task validation, and Windows smoke evidence | ORG-INT-01 | Read-only verification | Blocked | coordinator | Shared-wrapper suites pass decision repository 4/4, repository 31/31, task flow 6/6, and preview 4/4; frontend/assets/organize/legacy suite passes 36/36 with 72 assertions; generated-type check and task validation pass. Full interface typecheck remains blocked by pre-existing missing assets/package declarations, and live browser/release-bundle smoke is unavailable |
...
- [ ] Focused unit, contract, integration, generated-type, task-validator, and Windows smoke checks pass. Final shared-wrapper Rust suites, Tauri legacy tests, task validation, and frontend/assets/organize tests pass, with the component preview test now loading under Bun through the guarded Vite glob helper. Full interface typecheck is blocked by pre-existing missing `packages/assets` image/SVG modules plus unrelated `maplibre-gl` and `@spacebot/api-client` declarations; live browser/release-bundle smoke is unavailable.
...
- The task page now exposes current-library Locations, physical pinned Paths from the current Space layout, and the native Windows directory picker. Recent task destinations remain unavailable because the generated organize contract exposes no source query for them; the UI intentionally does not fabricate those rows. Severity: P2 capability gap, not a data-safety blocker.
