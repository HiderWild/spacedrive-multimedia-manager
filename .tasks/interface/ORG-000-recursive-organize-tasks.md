...
| ORG-FE-02 | Virtualized direct children, Ctrl/Shift/Ctrl+A, correct lasso, and selection tests | ORG-FE-01 | Organize selection, lasso, virtualization, and card files only | Merged | coordinator/FE-02 | `2e21aa211`, `e14c7127a`, `1a99b55a7`, `831ed4ef8`, `3880751b9`, `8b4d3730a`, `411e31d60`, `9326d4a13`; UUID-based selection, real row virtualization, rendered-item thumbnails, cursor pagination, current-filter Ctrl+A, directory progress, plain replacement, Ctrl union from pointer-down baseline, and edge-scroll wiring are covered by Bun tests |
...
| ORG-VERIFY-02 | Final build, focused suites, type drift, task validation, and Windows smoke evidence | ORG-INT-01 | Read-only verification | Blocked | coordinator | Shared-wrapper `sd-core --lib` passes 414/414 runnable tests, and focused integration suites pass 47/47 (`organize_repository` 33/33, `organize_decision_repository` 4/4, `organize_task_flow` 6/6, `preview_sequence` 4/4). Frontend organize suite passes 32/32 with 65 assertions; real type-generation drift check passes after the WSL/Git Bash path fix; task validation previously passes. Full interface typecheck remains blocked by pre-existing missing assets/package declarations, and live browser/release-bundle smoke is unavailable |
