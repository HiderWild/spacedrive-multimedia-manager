...
| ORG-BE-02 | SQLite migration, entities, task repository, and persistence tests | ORG-BE-01, ORG-BUILD-00, ORG-BUILD-BASELINE | Core migration and organize persistence files only | In Flight | persistence agent | Implementation may proceed in parallel with baseline repair; Cargo verification waits for ORG-BUILD-BASELINE |
...
| ORG-BUILD-BASELINE | Repair pre-existing sd-core compile blockers required for focused Rust verification | ORG-BUILD-00C | In Flight | baseline-repair agent | Known blockers: content_identity ActiveModel fields, ai_exclusion PaginatorTrait, ai_exclusion migration unique_key API, extension manager constructor/signature drift; modify only these existing files and add no feature semantics |
