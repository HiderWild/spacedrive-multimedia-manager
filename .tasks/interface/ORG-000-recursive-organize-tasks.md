...
| ORG-BE-01 | Windows path identity, tree intervals, sparse decisions, progress, and topology rules | ORG-PLAN, ORG-BUILD-00 | Core organize domain files and colocated tests only | In Flight | Poincare | Spec review found six P1 semantic gaps: same-source move, normalized Move destination equality, self/batch decision handling, multi-root rejection, and non-Windows test gating; fix and re-review before unlock |
