...
| ORG-INT-01 | Vertical smoke flow for create, mark, review, commit, restart, and drift | ORG-FE-04, ORG-BE-05 | Integration tests and minimal test fixtures | Verified | coordinator | Final WebDriver run against Edge WebView2 `151.0.4129.107` passed 5/5: app connection, Tauri API, daemon status, recursive task with nested decisions/conflict safety/move/permanent delete/Finish/Reopen, and changed-source drift protection. Selenium `4.48.0` used Selenium Manager's matching driver; temporary task records and filesystem fixtures were cleaned up |
...
- [x] Focused unit, contract, integration, generated-type, task-validator, and Windows smoke checks pass. Final shared-wrapper Rust suites, Tauri legacy tests, task validation, combined frontend/preview tests, the full interface typecheck, and the real Windows WebDriver/Tauri smoke check pass.
