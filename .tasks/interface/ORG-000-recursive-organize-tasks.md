...
| ORG-BUILD-00B-PW | Wire PowerShell start/run/restart/cache entry points | ORG-BUILD-00A | In Flight | Turing | Spec and quality reviews returned the same P1: reachable Tauri dev/package chain still bypasses wrapper via `bun run tauri:dev` -> `dev-with-daemon.ts` and direct Tauri Cargo paths; fix complete transitive chain and extend repository fixtures before re-review |
| ORG-BUILD-00B-SH | Wire justfile, Tauri package, and shell Cargo entry points | ORG-BUILD-00A | In Flight | Feynman | Spec approved; quality review returned P1: `just dev-desktop`, `tauri:dev`, `tauri:dev:no-watch`, `tauri:build`, config hooks and `dev-with-daemon.ts` remain reachable bypasses; coordinate with PowerShell lane and re-review |
