...
| ORG-INT-01 | Vertical smoke flow for create, mark, review, commit, restart, and drift | ORG-FE-04, ORG-BE-05 | Integration tests and minimal test fixtures | Candidate | coordinator | WebDriver harness migrated in `80ae1ac53` and legacy dead code removed in `a49502b63`; Windows snapshot fixtures added in `201988ca3`; live browser/release-bundle execution remains unavailable, and the shared-wrapper verification is being rerun after the schema fix |
...
| ORG-VERIFY-02 | Final build, focused suites, type drift, task validation, and Windows smoke evidence | ORG-INT-01 | Read-only verification | Blocked | coordinator | Shared-wrapper suites pass decision repository 4/4, repository 31/31, task flow 6/6, and preview 4/4; frontend organize/legacy suite passes 29/29; generated-type check and task validation pass. Full interface typecheck remains blocked by pre-existing missing assets/package declarations, the component preview test hits the existing `@sd/assets/svgs/ext/Extras/urls` Bun resolution gap, and live browser/release-bundle smoke is unavailable |
...
- [ ] Focused unit, contract, integration, generated-type, task-validator, and Windows smoke checks pass. Final shared-wrapper Rust suites, Tauri legacy tests, task validation, and frontend organize tests pass. The component preview test is blocked by the existing `@sd/assets/svgs/ext/Extras/urls` Bun resolution gap. Full interface typecheck is blocked by pre-existing missing `packages/assets` image/SVG modules plus unrelated `maplibre-gl` and `@spacebot/api-client` declarations; live browser/release-bundle smoke is unavailable.
...
- Full `packages/interface` typecheck still reports the repository's pre-existing missing `packages/assets` image/SVG modules, `maplibre-gl/dist/maplibre-gl.css`, and `@spacebot/api-client` declarations. The current run exits 1 without organize-route errors. Trigger: restore the missing assets/declarations before claiming a green full typecheck. Severity: P2, workaround: use the 29-test organize/legacy suite and organize-only error filter.
- The organize preview component test cannot load the existing `@sd/assets/svgs/ext/Extras/urls` import under Bun, although the backend preview sequence suite passes 4/4 and the focused media poster tests pass 2/2. Trigger: make the assets package export resolvable by the test/runtime toolchain. Severity: P2 infrastructure gap, not an organize contract failure.
- The task page now exposes current-library Locations and the native Windows directory picker. Recent task destinations and pinned physical Paths remain unavailable because the generated contract exposes no source query for them; the UI intentionally does not fabricate those rows. Severity: P2 capability gap, not a data-safety blocker.
- Change acceptance currently lists the first 200 root-level child records returned by the existing `organize.children` contract. If a change exists deeper in the snapshot and is not in that page, the panel shows an unavailable-details message rather than offering a misleading partial accept-all action. A recursive change-details query is a future contract decision, not silently added to this implementation.
