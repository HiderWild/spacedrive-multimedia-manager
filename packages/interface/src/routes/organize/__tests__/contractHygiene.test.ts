...
function productionSource(root: string, extensions: readonly string[]): string[] {
	return sourceFiles(root, extensions).filter(
		(path) => !path.includes("__tests__") && !path.endsWith(".test.ts") && !path.endsWith(".test.tsx"),
	);
}

function readSources(roots: readonly string[], extensions: readonly string[]): string {
	return roots
...
describe("recursive organize retirement contract", () => {
	...
	test("old organize route is removed without deleting shared QuickPreview", () => {
		expect(() => statSync("packages/interface/src/routes/explorer/organize")).toThrow();
		expect(statSync("packages/interface/src/components/QuickPreview").isDirectory()).toBe(true);
		expect(statSync("packages/interface/src/routes/organize").isDirectory()).toBe(true);
	});
});
