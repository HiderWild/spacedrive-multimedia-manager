	...
	return (
		<main className="flex h-full min-h-0 flex-col text-ink">
			...
			<div className="flex min-h-0 flex-1">
				<section className="min-w-0 flex-1 overflow-auto p-4" tabIndex={0} onScroll={(event) => setScrollTop(event.currentTarget.scrollTop)} onKeyDown={(event) => { if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 'a') { event.preventDefault(); select({type: 'selectAll', parentItemId: parentItemId ?? task.root_item_id, filter: 'All'}); } }}>
